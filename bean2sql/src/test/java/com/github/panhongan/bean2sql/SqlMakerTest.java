...
public class SqlMakerTest {
    ...
    public void testMakeQueryByConditionSql_TableNameIsEmpty() {
        SqlMaker.makeQueryByConditionSql("", null, null);
    }

    @Test(expected = NullPointerException.class)
    public void testMakeQueryByConditionSql_SqlConditionIsNull() {
        SqlMaker.makeQueryByConditionSql("t_person", null, null);
    }
    ...
    public void testMakeQueryByConditionSql_ClassIsNull() {
        ...
        SqlMaker.makeQueryByConditionSql("t_person", condition, null);
    }
    ...
    public void testMakeQueryByConditionSql_Ok() {
        ...
        Pair<String, Map<Integer, String>> pair = SqlMaker.makeQueryByConditionSql(TABLE_NAME, condition, obj.getClass());

        assert(pair.getLeft().contentEquals("select name,age,height,weight,date from t_person where (name=?)"));
        assert(pair.getRight().size() == 1);
    }

    @Test(expected = IllegalArgumentException.class)
    public void testMakeQueryByPageSql_TableNameIsEmpty() {
        SqlMaker.makeQueryByPageSql("", null, null, null);
    }

    @Test(expected = NullPointerException.class)
    public void testMakeQueryByPageSql_SqlConditionIsNull() {
        SqlMaker.makeQueryByPageSql("t_person", null, null, null);
    }
    ...
    public void testMakeQueryByCPageSql_PageContextIsNull() {
        ...
        EqualCondition<TestObj> condition = SqlConditionMaker.equalCondition(obj);
        SqlMaker.makeQueryByPageSql("t_person", condition, null, null);
    }
    ...
    public void testMakeQueryByCPageSql_ClassIsNull() {
        ...
        EqualCondition<TestObj> condition = SqlConditionMaker.equalCondition(obj);
        SqlMaker.makeQueryByPageSql("t_person", condition, pageContext, null);
    }
    ...
    public void testMakeQueryByPageSql_Ok() {
        ...
        Pair<Pair<String, String>, Map<Integer, String>> pair = SqlMaker.makeQueryByPageSql(TABLE_NAME, condition, pageContext, obj.getClass());

        assert(pair.getLeft().getLeft().contentEquals("select count(id) from t_person where (name=?)"));
        assert(pair.getLeft().getRight().contentEquals("select name,age,height,weight,date from t_person where (name=?) limit 0,10"));
        assert(pair.getRight().size() == 1);
    }
}
